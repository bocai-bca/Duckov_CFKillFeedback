...
namespace CFKillFeedback
{
	public class ModBehaviour : Duckov.Modding.ModBehaviour
	{
		...
		public const float ComboSeconds = 8.0f;
		...
		private void Update()
		{
			...
			Vector3 tr_pos = new Vector3();
			if (ui_canvasgroup != null && ui_transform != null)
			{
				Vector2 screen_size = ui_transform.parent.position * 2.0f;
				if (delta < IconDropTime)
				{
					float current_delta = Math.Clamp(delta / IconDropTime, 0.0f, 1.0f);
					ui_canvasgroup.alpha = current_delta * IconStayAlpha;
					ui_transform.localScale = Vector3.Lerp(IconSizeDrop, IconSizeStay, current_delta) * Math.Min(ui_transform.parent.position.x / IconSizeBaseMulti.x, ui_transform.parent.position.x / IconSizeBaseMulti.y);
					Vector3 pos = Vector3.Lerp(IconPosPrctDrop, IconPosPrctStay, current_delta);
					tr_pos = new Vector3(pos.x * screen_size.x, pos.y * screen_size.y);
				}
				else if (delta > IconDropTime + IconStayTime)
				{
					ui_transform.localScale = IconSizeStay * Math.Min(ui_transform.parent.position.x / IconSizeBaseMulti.x, ui_transform.parent.position.x / IconSizeBaseMulti.y);
					ui_canvasgroup.alpha = (1.0f - Math.Clamp((delta - IconDropTime - IconStayTime) / IconFadeoutTime, 0.0f, 1.0f)) * IconStayAlpha;
		...
		public void OnDead(Health health, DamageInfo damageInfo)
		{
			...
			if (health.IsMainCharacterHealth)
			{
				if (KillFeedbackAudios_FMOD.ContainsKey("death"))
				{
					...
					RuntimeManager.CoreSystem.playSound(KillFeedbackAudios_FMOD["death"], channel_group, false, out channel);
				}
				return;
			}
			// 如果伤害来自玩家队
		...
		public static void UpdateCombo()
		{
			...
			if (time - last_kill_time > (float)ComboSeconds || time - last_kill_time <= 0.0f)
			{
				// 重置连杀数为第一杀
				combo_count = 1;
			}
